...
function Animate() {
    ...
    this.init = function() {
        this.animationId = window.requestAnimationFrame(that.init.bind(this));
        if (pool.player1.turn) {
            ...
            p2.classList.remove('computer-turn');
            ...
            if (pool.player2.isComputer) p2.classList.add('computer-turn');
        }
        if (colorSet) {
        ...
        p1.innerHTML = `${document.getElementById('player1Name').textContent}: ${pool.player1.score}`;
        p2.innerHTML = gameMode === "PvC" ? `Computer: ${pool.player2.score}` : `${document.getElementById('player2Name').textContent}: ${pool.player2.score}`;
    ...
    this._initBlackBall = function() {
        ...
        this.displayFouledMessage = function() {
            ...
            if (pool.player1.fouled || pool.player2.fouled) {
                canvasText.style.display = 'inline';
                ...
                let currentPlayer = pool.player1.fouled ? pool.player1 : pool.player2;
                let opponentPlayer = pool.player1.fouled ? pool.player2 : pool.player1;
        ...
        this.checkIfBallsStopped = function() {
            ...
            if (!flag) {
                ...
                if (pool.player1.game || pool.player2.game) {
                    this.endGame();
                } else if (!collisionOccured && !initialFoulChecked) {
                    let currentPlayer = pool.player1.turn ? pool.player1 : pool.player2;
                    let opponentPlayer = pool.player1.turn ? pool.player2 : pool.player1;
                    currentPlayer.fouled = true;
                    currentPlayer.addScore(-1);
                    currentPlayer.turn = false;
                    opponentPlayer.turn = true;
                }
                // Removed the default turn switch here to allow extra shot logic in verifyFoul to take precedence
                    ...
                    if (pool.ballsArray[i].color == '#f03c3c') {
                        ...
                        do {
                            ...
                            for (var j = 1; j < pool.ballsArray.length; j++) {
                                var distSqr = (randomX - pool.ballsArray[j].x) * (randomX - pool.ballsArray[j].x) + 
                                              (randomY - pool.ballsArray[j].y) * (randomY - pool.ballsArray[j].y);
        ...
        this.checkIfPotted = function() {
            for (var i = 0; i < pool.ballsArray.length; i++) {
                if (((pool.ballsArray[i].x - (boundLeft - 35)) * (pool.ballsArray[i].x - (boundLeft - 35)) + 
                     (pool.ballsArray[i].y - (boundTop - 35)) * (pool.ballsArray[i].y - (boundTop - 35)) - 75 * 75) < 0) {
                ...
                } else if (((pool.ballsArray[i].x - (boundRight + 35)) * (pool.ballsArray[i].x - (boundRight + 35)) + 
                            (pool.ballsArray[i].y - (boundTop - 35)) * (pool.ballsArray[i].y - (boundTop - 35)) - 75 * 75) < 0) {
                    if (pool.ballsArray[i].statusFlag == 'inTable') {
                        pool.ballsArray[i].statusFlag = 'potted';
                        pool.ballsArray[i].vy = 0; pool.ballsArray[i].vx = 0;
                ...
                } else if (((pool.ballsArray[i].x - (boundLeft - 35)) * (pool.ballsArray[i].x - (boundLeft - 35)) + 
                            (pool.ballsArray[i].y - (boundBottom + 35)) * (pool.ballsArray[i].y - (boundBottom + 35)) - 75 * 75) < 0) {
                    if (pool.ballsArray[i].statusFlag == 'inTable') {
                        pool.ballsArray[i].statusFlag = 'potted';
                        pool.ballsArray[i].vy = 0; pool.ballsArray[i].vx = 0;
                ...
                } else if (((pool.ballsArray[i].x - canvas.width / 2) * (pool.ballsArray[i].x - canvas.width / 2) + 
                            (pool.ballsArray[i].y - (boundTop - 195)) * (pool.ballsArray[i].y - (boundTop - 195)) - 200 * 200) < 0) {
                    if (pool.ballsArray[i].statusFlag == 'inTable') {
                        pool.ballsArray[i].statusFlag = 'potted';
                        pool.ballsArray[i].vy = 0; pool.ballsArray[i].vx = 0;
                        ...
                        this.verifyFoul(i);
                            ...
                            (pool.ballsArray[i].y - (boundBottom + 195)) * (pool.ballsArray[i].y - (boundBottom + 195)) - 200 * 200) < 0) {
                    if (pool.ballsArray[i].statusFlag == 'inTable') {
                        pool.ballsArray[i].statusFlag = 'potted';
                        pool.ballsArray[i].vy = 0; pool.ballsArray[i].vx = 0;
        ...
        this.verifyFoul = function(i) {
            ...
            if (i == 0) { // White ball potted
                currentPlayer.fouled = true;
                currentPlayer.addScore(-1);
                currentPlayer.turn = false;
                opponentPlayer.turn = true;
            } else if (i == 1) { // Black ball potted
                if ((currentPlayer.color == '#f03c3c' && pool.redBalls.length == 0) || 
                    (currentPlayer.color == '#e6e616' && pool.yellowBalls.length == 0)) {
                    currentPlayer.addScore(10);
                    currentPlayer.game = true;
                } else {
                    opponentPlayer.addScore(10);
                    opponentPlayer.game = true;
                }
            } else { // Colored ball potted
                if (!colorSet) {
                    currentPlayer.color = pool.ballsArray[i].color;
                    opponentPlayer.color = currentPlayer.color == '#f03c3c' ? '#e6e616' : '#f03c3c';
                    colorSet = true;
                    currentPlayer.addScore(1);
                    // Extra shot: turn remains with current player
                } else if (currentPlayer.color == pool.ballsArray[i].color) {
                    currentPlayer.addScore(1);
                    // Extra shot: turn remains with current player, no switch
                } else {
                    currentPlayer.fouled = true;
                    currentPlayer.turn = false;
                    opponentPlayer.turn = true;
                    ...
                    if ((j != i) && (pool.ballsArray[i].statusFlag == 'inTable' && pool.ballsArray[j].statusFlag == 'inTable')) {
                        ...
                        if (distSquared <= (pool.ballsArray[i].radius + pool.ballsArray[j].radius) * (pool.ballsArray[i].radius + pool.ballsArray[j].radius)) {
                            ...
                            if (!initialFoulChecked) {
                                let currentPlayer = pool.player1.turn ? pool.player1 : pool.player2;
                                if (i == 0 && colorSet && currentPlayer.color != pool.ballsArray[j].color) {
                                    currentPlayer.fouled = true;
                                    currentPlayer.addScore(-1);
                                }
        ...
        this.endGame = function() {
            canvasText.style.display = 'inline';
            canvasText.innerHTML = pool.player1.game ? `${document.getElementById('player1Name').textContent} wins` : 
                                  (gameMode === "PvC" ? 'Computer wins' : `${document.getElementById('player2Name').textContent} wins`);
            window.cancelAnimationFrame(this.animationId);
            canvas.style.cursor = 'pointer';
            
            setTimeout(() => {
                document.getElementById('gameContainer').style.display = 'none';
                showRestartPage(pool.player1.game ? document.getElementById('player1Name').textContent : 
                                (gameMode === "PvC" ? 'Computer' : document.getElementById('player2Name').textContent));
            }, 3000);
        }
                                               }
